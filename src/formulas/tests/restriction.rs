mod restriction_tests {
    ...
    fn ass(ff: &F) -> Assignment {
        Assignment::from_variables(
            ...
            &[ff.B.as_variable().unwrap(), ff.X.as_variable().unwrap()],
        )
    }
    ...
    fn test_restrict_pbc() {
        ...
        assert_eq!(
            f.restrict(pb1, &a1),
            f.pbc(CType::EQ, 0, lits_a1, coeffs_a1).unwrap()
        );
        assert_eq!(
            f.restrict(pb1, &a2),
            f.pbc(CType::EQ, 2, lits_a2, coeffs_a2).unwrap()
        );
