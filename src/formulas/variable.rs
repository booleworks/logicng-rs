...
/// The prefix for CNF encoding auxiliary variables
pub const AUX_CNF: &str = "CNF";
/// The prefix for cardinality encoding constraint auxiliary variables
pub const AUX_CC: &str = "CC";
/// The prefix for pseudo-boolean encoding constraint auxiliary variables
...
impl TryFrom<FormulaEncoding> for Variable {
    ...
    fn try_from(enc: FormulaEncoding) -> Result<Self, Self::Error> {
        match enc.formula_type() {
            FormulaType::Lit(LitType::Pos | LitType::Neg) => Ok(Self(FormulaEncoding::encode(
                enc.index(),
                FormulaType::Lit(LitType::Pos),
                true,
            ))),
            _ => Err(format!(
                "Cannot convert {:?} to an variable!",
                enc.formula_type()
            )),
        }
    }
