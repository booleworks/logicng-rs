...
pub fn encode_swc(lits: &[Literal], coeffs: &[usize], rhs: usize, f: &FormulaFactory) -> Vec<EncodedFormula> {
    ...
    for i in 0..n {
        ...
        for j in 0..rhs {
            ...
            if i >= 1 && j < rhs - ci {
                result.push(f.clause([seq_auxiliary[i - 1][j].neg_lit(), lits[i].negate(), seq_auxiliary[i][j + ci].pos_lit()]));
            }
        }
        if i >= 1 {
