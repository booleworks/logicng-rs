...
impl Display for LngEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            Self::MaxSatNewLowerBound(lb) => f.write_fmt(format_args!("new MaxSAT lower bound {lb}")),
            Self::MaxSatNewUpperBound(ub) => f.write_fmt(format_args!("new MaxSAT upper bound {ub}")),
