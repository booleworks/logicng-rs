...
use crate::{
    errors::LngResult,
    explanations::{UnsatCore, drup_compute},
...
pub fn compute_unsat_core<B: PartialEq>(
    ...
    if result.trivial_unsat {
        ...
        let propositions = result
            ...
            .dedup()
            .collect();
        Ok(UnsatCore::new(propositions, false))
    }
}

fn handle_trivial_case<B: PartialEq>(
    solver: &mut SatSolver<B>,
    f: &FormulaFactory,
) -> LngResult<UnsatCore<B>> {
    ...
    for i in 0..clauses.len() {
        ...
        for cj in clauses.iter().skip(i + 1) {
            if ci.clause.len() == 1 && cj.clause.len() == 1 && ci.clause[0] + cj.clause[0] == 0 {
                ...
                return Ok(UnsatCore::new(
                    if pi == pj { vec![pi] } else { vec![pi, pj] },
                    false,
                ));
            }
        }
...
fn get_formula_for_vector<B>(
    ...
    for &lit in vector {
        let var = solver
            .underlying_solver()
            .variable_for_idx(LngVar(lit.unsigned_abs() - 1), f);
