...
use crate::{
    explanations::{drup_compute, UnsatCore},
    formulas::{EncodedFormula, FormulaFactory, Literal},
    propositions::Proposition,
    solver::lng_core_solver::{LngVar, SatSolver},
};

pub fn compute_unsat_core<B: PartialEq>(solver: &mut SatSolver<B>, f: &FormulaFactory) -> UnsatCore<B> {
    ...
    if result.trivial_unsat {
        ...
        let propositions =
            result.unsat_core.into_iter().map(|c| clause2propositions[&get_formula_for_vector(solver, &c, f)].clone()).dedup().collect();
        UnsatCore::new(propositions, false)
    }
}

fn handle_trivial_case<B: PartialEq>(solver: &mut SatSolver<B>, f: &FormulaFactory) -> UnsatCore<B> {
...
fn get_formula_for_vector<B>(solver: &mut SatSolver<B>, vector: &[isize], f: &FormulaFactory) -> EncodedFormula {
    let mut literals = Vec::with_capacity(vector.len());
    for &lit in vector {
        let var = solver.underlying_solver().variable_for_idx(LngVar(lit.unsigned_abs() - 1), f);
