...
use super::{
    BoundedQueue, ClauseMinimization, ClauseRef, LngClause, LngHeap, LngLit, LngState, LngVar,
    LngVariable, LngWatcher, ProofInformation, SatSolverConfig, SolverState, Tristate, mk_lit,
    sign, var,
};
...
impl LngCoreSolver {
    ...
    pub fn idx_for_variable(&self, variable: Variable) -> Option<LngVar> {
        self.var2idx
            .get(&variable)
            .or_else(|| self.aux2idx.get(&variable))
    ...
    pub fn add_clause(&mut self, mut ps: Vec<LngLit>, proposition: Option<PropositionID>) -> bool {
        ...
        let flag =
            self.config.proof_generation && ps.iter().any(|&l| self.value(l) != Tristate::Undef);
        ...
        if flag {
            ...
            if let Some(original_clause) = oc {
                let mut vec = Vec::<isize>::with_capacity(original_clause.len() + 1);
                vec.push(-1);
                for elem in original_clause {
                    vec.push((var(elem).0 as isize + 1) * (-2 * isize::from(sign(elem)) + 1));
                }
                self.pg_proof.push(vec);
            }
    ...
    pub(crate) fn internal_solve(
    ...
    ) -> CancelableResult<bool> {
        if !handler.should_resume(LngEvent::ComputationStarted(LngComputation::Sat)) {
            return CancelableResult::Canceled(LngEvent::ComputationStarted(LngComputation::Sat));
        }

        self.model.clear();
        self.assumptions_conflict.clear();
        if !self.ok {
            return CancelableResult::Ok(false);
        }
        let mut status = CancelableResult::Ok(Tristate::Undef);
        while matches!(status, CancelableResult::Ok(Tristate::Undef)) {
            status = self.search(handler);
        }

        let result = match status {
            CancelableResult::Ok(s) => s == Tristate::True,
            CancelableResult::Canceled(lng_event) | CancelableResult::Partial(_, lng_event) => {
                self.cancel_until(0);
                return CancelableResult::Canceled(lng_event);
            }
        };
        ...
        if !handler.should_resume(LngEvent::ComputationFinished(LngComputation::Sat)) {
            return CancelableResult::Canceled(LngEvent::ComputationFinished(LngComputation::Sat));
        }
        CancelableResult::Ok(result)
    }

    /// Solves the formula on this solver with a list of assumptions.
    pub(crate) fn internal_solve_with_assumptions(
        &mut self,
        handler: &mut dyn ComputationHandler,
    ...
    pub fn save_state(&mut self) -> SolverState {
        ...
        SolverState {
            ...
            pg_original_size: if self.config.proof_generation {
                self.pg_original_clauses.len()
            } else {
                0
            },
            pg_proof_size: if self.config.proof_generation {
                self.pg_proof.len()
            } else {
                0
            },
    ...
    pub fn load_state(&mut self, solver_state: &SolverState) -> Result<(), Box<dyn Error>> {
        ...
        let Some(index) = index else {
            return Err("The given solver state is not valid anymore".into());
        };
        ...
        if self.config.proof_generation {
            let new_pg_original_size = min(
                solver_state.pg_original_size,
                self.pg_original_clauses.len(),
            );
    ...
    pub(crate) fn pick_branch_lit(&mut self) -> LngLit {
        ...
        while next.is_none()
            || self.vars[next.unwrap().0].assignment != Tristate::Undef
    ...
    pub(crate) fn unchecked_enqueue(&mut self, lit: LngLit, reason: Option<ClauseRef>) {
        ...
        if let Some(assignment) = &mut self.dnnf_assignment {
            assignment[lit.0] = Tristate::True;
            assignment[not(lit).0] = Tristate::False;
        }
        ...
        if clause.is_at_most {
            if let Some(at_most_watchers) = clause.at_most_watchers {
                for l in &clause.data[..at_most_watchers] {
                    self.watches[l.0].push(LngWatcher {
                        clause_ref,
                        blocking_literal: LngLit::UNDEF,
                    });
                }
            }
            ...
            if clause.len() == 2 {
                self.watches_bin[not(lit0).0].push(LngWatcher {
                    clause_ref,
                    blocking_literal: lit1,
                });
                self.watches_bin[not(lit1).0].push(LngWatcher {
                    clause_ref,
                    blocking_literal: lit0,
                });
            } else {
                self.watches[not(lit0).0].push(LngWatcher {
                    clause_ref,
                    blocking_literal: lit1,
                });
                self.watches[not(lit1).0].push(LngWatcher {
                    clause_ref,
                    blocking_literal: lit0,
                });
            }
            ...
            while i_ind < self.watches[p.0].len() {
                ...
                if self.c(c_ref).is_at_most {
                        ...
                        if let Some(at_most_watchers) = self.c(c_ref).at_most_watchers {
                            for k in 0..at_most_watchers {
                                ...
                                if w != p && self.value(w) != Tristate::False {
                                    assert!(
                                        self.value(w) == Tristate::Undef
                                            || self.value(w) == Tristate::False
                                    );
                    ...
                    if first != blocker && self.value(first) == Tristate::True {
                        self.watches[p.0][j_ind] = LngWatcher {
                            clause_ref: c_ref,
                            blocking_literal: first,
                        };
                        j_ind += 1;
                        continue;
                    }
                    ...
                    for k in 2..c.len() {
                        ...
                        if self.value(k_lit) != Tristate::False {
                            ...
                            let c = &self.all_clauses[c_ref.0];
                            self.watches[not(c.get(1)).0].push(LngWatcher {
                                clause_ref: c_ref,
                                blocking_literal: first,
                            });
                            found_watch = true;
    ...
    pub(crate) fn lit_redundant(
        ...
        analyze_to_clear: &mut Vec<LngLit>,
    ) -> bool {
            ...
            if c.is_at_most {
                for &l in &c.data {
                    ...
                    if !self.seen[var(q).0] && self.v(q).level_greater_zero() {
                        if self.v(q).reason.is_some()
                            && (self.abstract_level(var(q)) & abstract_levels) != 0
                        {
    ...
    pub(crate) fn cancel_until(&mut self, level: usize) {
        if self.decision_level() > level {
            for c in (self.trail_lim[level]..self.trail.len()).rev() {
                ...
                let x = var(lit);
                if let Some(assignment) = &mut self.dnnf_assignment {
                    assignment[lit.0] = Tristate::Undef;
                    assignment[not(lit).0] = Tristate::Undef;
                }
    ...
    pub(crate) fn reduce_db(&mut self) {
        ...
        if self
            .c(self.learnts[self.learnts.len() / RATIO_REMOVE_CLAUSES])
            .lbd
            <= 3
        {
        ...
        if self
            .c(*self.learnts.last().expect("expected learnt clause"))
            .lbd
            <= 5
        {
            self.nb_clauses_before_reduce += self.config.low_level_config().special_inc_reduce_db();
    ...
    #[allow(clippy::too_many_lines)]
    fn search(&mut self, handler: &mut dyn ComputationHandler) -> CancelableResult<Tristate> {
        if !self.ok {
            return CancelableResult::Ok(Tristate::False);
        }
        self.selection_order_idx = 0;
        loop {
            if let Some(confl) = self.propagate() {
                if !handler.should_resume(LngEvent::SatConflictDetected) {
                    return CancelableResult::Canceled(LngEvent::SatConflictDetected);
                }
                self.conflicts += 1;
                self.conflicts_restarts += 1;
                if self.conflicts % 5000 == 0
                    && self.var_decay < self.config.low_level_config().max_var_decay()
                {
                    self.var_decay += 0.01;
                }
                if self.decision_level() == 0 {
                ...
                if self.conflicts_restarts > LB_BLOCKING_RESTART
                    ...
                    && self.trail.len() as f64
                        > self.config.low_level_config().factor_r() * self.trail_queue.avg() as f64
                {
                ...
                if learnt_clause.len() == 1 {
                    ...
                    let mut cr =
                        LngClause::new(learnt_clause, Some(LngState(self.next_state_id)), false);
                        ...
                        > (self.sum_lbd as f64 / self.conflicts_restarts as f64)
                {
                    self.lbd_queue.fast_clear();
                    self.cancel_until(0);
                    return CancelableResult::Ok(Tristate::Undef);
                }
                if self.conflicts >= (self.cur_restart * self.nb_clauses_before_reduce)
                ...
                while self.decision_level() < self.assumptions.len() {
                    ...
                    match self.value(p) {
                        ...
                        Tristate::False => {
                            if self.config.proof_generation {
                                let drup_lit =
                                    (var(p).0 as isize + 1) * (-2 * isize::from(sign(p)) + 1);
                                self.pg_original_clauses.push(ProofInformation::new(
                                    Vec::from([drup_lit]),
                                    self.assumption_propositions
                                        .get(self.decision_level())
                                        .cloned(),
                                ));
                            }
                ...
                if next == LngLit::UNDEF {
                    ...
                    if next == LngLit::UNDEF {
                        return CancelableResult::Ok(Tristate::True);
                    }
                }
                ...
                for q in start..end {
                    ...
                    if !self.seen[var(data_lit).0] && self.v(data_lit).level_greater_zero() {
                        ...
                        if self
                            .v(data_lit)
                            .level
                            .is_some_and(|l| l >= self.decision_level())
                        {
                            path_c += 1;
                            if self
                                .v(data_lit)
                                .reason
                                .is_some_and(|r| self.c(r).learnt_on_state.is_some())
                            {
        ...
        match self.config.clause_minimization() {
            ClauseMinimization::Deep => {
                ...
                for i in 1..out_learnt.len() {
                    ...
                    if self.v(i_lit).reason.is_none()
    ...
    pub(crate) fn complete_backtrack(&mut self) {
        if let Some(assignment) = &mut self.dnnf_assignment {
            assignment.fill(Tristate::Undef);
        }
    ...
    pub(crate) fn simple_remove_clause(&mut self, clause: ClauseRef) {
        ...
        if c.is_at_most {
            for i in 0..c.at_most_watchers.expect("expected at-most watchers") {
                let lit = c.get(i);
                let compare = LngWatcher {
                    clause_ref: clause,
                    blocking_literal: lit,
                };
                let index = self.watches[lit.0]
                    .iter()
                    .find_position(|e| e.clause_ref == compare.clause_ref);
            ...
            let compare = LngWatcher {
                clause_ref: clause,
                blocking_literal: c.get(1),
            };
            let index = self.watches_bin[lit.0]
                .iter()
                .find_position(|e| e.clause_ref == compare.clause_ref);
            if let Some((index, _)) = index {
    ...
    pub(crate) fn find_new_watch_for_at_most_clause(
        &mut self,
        clause: ClauseRef,
        p: LngLit,
    ) -> LngLit {
        ...
        V: Into<Variable>,
    {
        if !handler.should_resume(LngEvent::ComputationStarted(LngComputation::Backbone)) {
            return CancelableResult::Canceled(LngEvent::ComputationStarted(
                LngComputation::Backbone,
            ));
        }
        ...
        let result = if !solve_result.is_success() {
            CancelableResult::Canceled(solve_result.cancel_cause().unwrap())
        } else if solve_result.result().unwrap() {
            self.computing_backbone = true;
            let relevant_var_indices = self.get_relevant_var_indices(variables.clone());
            self.init_backbone_ds(&relevant_var_indices);
            let backbone_event =
                self.compute_backbone_intern(&relevant_var_indices, backbone_type, handler);
            let r = backbone_event.map_or_else(
                || CancelableResult::Ok(self.build_backbone(variables, backbone_type)),
                CancelableResult::Canceled,
            );
            self.computing_backbone = false;
            r
        } else {
            CancelableResult::Ok(Backbone::new_unsat())
        };
        self.load_state(&state_before_backbone)
            .expect("Controlled creation of state");
        result
    }
        ...
        V: Into<Variable>,
    {
        variables
            .into_iter()
            .filter_map(|v| self.idx_for_variable(v.into()))
            .collect()
    }
    ...
    pub(crate) fn create_initial_candidates(
        &mut self,
        variables: &[LngVar],
        backbone_type: BackboneType,
    ) {
        for &var in variables {
            if self.is_up_zero_lit(var) {
                ...
                if is_both_or_negative_type(backbone_type) && !model_phase
                    || is_both_or_positive_type(backbone_type) && model_phase
                {
    ...
    pub(crate) fn solve_with_lit(
        ...
        let result =
            self.internal_solve_with_assumptions(handler, self.backbone_assumptions.clone());
    ...
    where
        ...
        I: IntoIterator<Item = V>,
    {
        let mut pos = if backbone_type == BackboneType::OnlyNegative {
            None
        } else {
            Some(BTreeSet::new())
        };
        let mut neg = if backbone_type == BackboneType::OnlyPositive {
            None
        } else {
            Some(BTreeSet::new())
        };
        let mut opt = if backbone_type == BackboneType::PositiveAndNegative {
            Some(BTreeSet::new())
        } else {
            None
        };
    ...
    pub(crate) fn manual_sort_learnts(&mut self) {
    ...
    fn manual_sort_learnts_rec(&mut self, start: usize, end: usize) {
        ...
        if end - start <= 15 {
            ...
            let (pivot_len, pivot_lbd, pivot_activity) = {
                let pivot_ref = self.learnts[start + (end - start) / 2];
                let pivot = self.c(pivot_ref);
                (pivot.len(), pivot.lbd, pivot.activity)
            };
            ...
            loop {
                loop {
                    i += 1;
                    let clause = self.c(self.learnts[i as usize]);
                    if Self::compare_learnt_clauses_2(
                        clause.len(),
                        clause.lbd,
                        clause.activity,
                        pivot_len,
                ...
                loop {
                    j -= 1;
                    let clause = self.c(self.learnts[j as usize]);
                    if Self::compare_learnt_clauses_2(
                        pivot_len,
                        pivot_lbd,
                        pivot_activity,
    ...
    fn selection_sort(&mut self, start: usize, end: usize) {
        ...
        for i in start..end {
            best_i = i;
            for j in (i + 1)..end {
                let candidate = &self.c(self.learnts[j]);
                let best = &self.c(self.learnts[best_i]);
                if Self::compare_learnt_clauses(candidate, best) == Less {
                    best_i = j;
                }
            }
            if i != best_i {
    ...
    fn compare_learnt_clauses(x: &LngClause, y: &LngClause) -> Ordering {
        Self::compare_learnt_clauses_2(x.len(), x.lbd, x.activity, y.len(), y.lbd, y.activity)
    }
...
pub const fn is_both_or_positive_type(backbone_type: BackboneType) -> bool {
    matches!(
        backbone_type,
        BackboneType::PositiveAndNegative | BackboneType::OnlyPositive
    )
}
...
pub const fn is_both_or_negative_type(backbone_type: BackboneType) -> bool {
    matches!(
        backbone_type,
        BackboneType::PositiveAndNegative | BackboneType::OnlyNegative
    )
}
