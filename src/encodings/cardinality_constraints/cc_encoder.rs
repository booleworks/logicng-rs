...
impl CcEncoder {
    ...
    pub fn encode_on(&self, result: &mut dyn EncodingResult, cc: &CardinalityConstraint) {
        self.encode_constraint(cc, result);
    }

    /// Encodes an incremental cardinality constraint in a given result.
    pub fn encode_incremental(&self, result: &mut dyn EncodingResult, cc: &CardinalityConstraint) -> Option<CcIncrementalData> {
        self.encode_incremental_constraint(cc, result)
    }

    fn encode_constraint(&self, cc: &CardinalityConstraint, result: &mut dyn EncodingResult) {
        let rhs = cc
            .rhs
            .try_into()
    ...
    fn amo(&self, result: &mut dyn EncodingResult, vars: &[Variable]) {
        if vars.len() <= 1 {
    ...
    fn amk(&self, result: &mut dyn EncodingResult, vars: &[Variable], rhs: usize, with_inc: bool) -> Option<CcIncrementalData> {
        if rhs >= vars.len() {
            ...
            vars.iter().for_each(|v| result.add_clause_literals(&[v.neg_lit()]));
            None
        } else {
    ...
    fn alk(&self, result: &mut dyn EncodingResult, vars: &[Variable], rhs: usize, with_inc: bool) -> Option<CcIncrementalData> {
        if rhs > vars.len() {
            result.add_clause_literals(&Vec::new());
            None
        } else if rhs == 0 {
            // there is no constraint
            None
        } else if rhs == 1 {
            result.add_clause_literals(&vars.iter().map(Variable::pos_lit).collect::<Vec<Literal>>());
            None
        } else if rhs == vars.len() {
            vars.iter().for_each(|v| result.add_clause_literals(&[v.pos_lit()]));
            None
        } else {
    ...
    fn exo(&self, result: &mut dyn EncodingResult, vars: &[Variable]) {
        if vars.is_empty() {
            result.add_clause(&Vec::new());
        } else if vars.len() == 1 {
            result.add_clause_literals(&[vars[0].pos_lit()]);
        } else {
            let lits: Vec<Literal> = vars.iter().map(Variable::pos_lit).collect();
            self.amo(result, vars);
            result.add_clause_literals(&lits);
        }
    }

    fn exk(&self, result: &mut dyn EncodingResult, vars: &[Variable], rhs: usize) {
        if rhs > vars.len() {
            result.add_clause_literals(&Vec::new());
        } else if rhs == 0 {
            for var in vars {
                result.add_clause_literals(&[var.neg_lit()]);
            }
        } else if rhs == vars.len() {
            for var in vars {
                result.add_clause_literals(&[var.pos_lit()]);
            }
        } else {
