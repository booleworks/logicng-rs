#![allow(clippy::cast_possible_truncation, clippy::cast_sign_loss, clippy::cast_precision_loss, clippy::cast_possible_wrap)]

use crate::datastructures::{EncodingResult, SolverExportLiteral};
use crate::formulas::{Literal, Variable, AUX_CC};

/// An encoding of at-most-one cardinality constraints using the binary
/// encoding due to Doggett, Frisch, Peugniez, and Nightingale.
pub fn build_amo_binary(result: &mut dyn EncodingResult, vars: &[Variable]) {
    let vars: Vec<SolverExportLiteral> = vars.iter().map(|var| Literal::new(*var, true).into()).collect();
    let number_of_bits = (vars.len() as f64).log2().ceil() as u32;
    let two_pow_n_bits = 2u64.pow(number_of_bits);
    let k = ((two_pow_n_bits - vars.len() as u64) * 2) as isize;
    let bits: Vec<SolverExportLiteral> = (0..number_of_bits).map(|_| result.new_auxiliary_variable(AUX_CC)).collect();

    let mut gray_code: isize;
    let mut next_gray: isize;
    let mut i = 0isize;
    let mut index = -1isize;
    while i < k {
        index += 1;
        gray_code = i ^ (i >> 1);
        i += 1;
        next_gray = i ^ (i >> 1);
        for j in 0..number_of_bits {
            if (gray_code & (1 << j)) == (next_gray & (1 << j)) {
                if (gray_code & (1 << j)) == 0 {
                    result.add_clause(&[vars[index as usize].negate(), bits[j as usize].negate()]);
                } else {
                    result.add_clause(&[vars[index as usize].negate(), bits[j as usize]]);
                }
            }
        }
        i += 1;
    }
    while i < two_pow_n_bits as isize {
        index += 1;
        gray_code = i ^ (i >> 1);
        for j in 0..number_of_bits {
            if (gray_code & (1 << j)) == 0 {
                result.add_clause(&[vars[index as usize].negate(), bits[j as usize].negate()]);
            } else {
                result.add_clause(&[vars[index as usize].negate(), bits[j as usize]]);
            }
        }
        i += 1;
    }
}
